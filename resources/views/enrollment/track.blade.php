...
@endphp
<div class="container py-5" id="enrollmentFlow" data-current-step="{{ $currentStep }}">
        ...
        <div class="col-lg-10">
            ...
            <div class="enrollment-tracker mb-5 px-4">
                <div class="d-flex justify-content-between position-relative">
                    ...
                    <div class="tracker-step {{ $currentStep >= 1 ? 'active' : '' }}" data-step="1" style="position: relative; z-index: 2; text-align: center; width: 120px;">
                        <div class="step-icon">1</div>
                        <span class="d-block small fw-bold mt-2">Selected Program</span>
                    </div>
                    <div class="tracker-step {{ $currentStep >= 2 ? 'active' : '' }}" data-step="2" style="position: relative; z-index: 2; text-align: center; width: 120px;">
                        <div class="step-icon">2</div>
                        <span class="d-block small mt-2">Your Details</span>
                    </div>
                    <div class="tracker-step {{ $currentStep >= 3 ? 'active' : '' }}" data-step="3" style="position: relative; z-index: 2; text-align: center; width: 120px;">
                        <div class="step-icon">3</div>
                        <span class="d-block small mt-2">Payment</span>
                    </div>
                    <div class="tracker-step {{ $currentStep >= 4 ? 'active' : '' }}" data-step="4" style="position: relative; z-index: 2; text-align: center; width: 120px;">
            ...
            <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
                <form action="{{ route('user.enrollment.store', $program->id) }}" method="POST" class="card-body p-4 p-md-5">
                    ...
                    <div class="enrollment-step-content {{ $currentStep === 1 ? '' : 'd-none' }}" id="step-1-content">
                        ...
                        @if($existingEnrollment && $currentStep !== 4)
                    ...
                    <!-- Step 2: Your Details -->
                    <div class="enrollment-step-content {{ $currentStep === 2 ? '' : 'd-none' }}" id="step-2-content">
                    ...
                    <!-- Step 3: Payment -->
                    <div class="enrollment-step-content {{ $currentStep === 3 ? '' : 'd-none' }}" id="step-3-content">
                        ...
                        <div class="form-check mb-2">
                            ...
                            <label class="form-check-label" for="termsAccepted">
                                I confirm my enrollment details are correct and I accept the program terms.
                            </label>
                        </div>
                        <div class="mt-5 d-flex gap-2">
                    ...
                    <!-- Step 4: Confirmation -->
                    <div class="enrollment-step-content {{ $currentStep === 4 ? '' : 'd-none' }}" id="step-4-content">
                        <div class="text-center py-4">
                            ...
                            @if($existingEnrollment)
                                <p class="small text-muted mb-4">Reference: #{{ $existingEnrollment->id }} | Submitted {{ optional($existingEnrollment->submitted_at)->format('M d, Y h:i A') ?? 'just now' }}</p>
                            @endif
                            <a href="{{ route('user.dashboard') }}" class="btn btn-primary rounded-pill px-5 fw-bold">Go to My Dashboard</a>
                        </div>
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    const enrollmentFlow = document.getElementById('enrollmentFlow');
    const nextBtns = document.querySelectorAll('.next-step');
    const prevBtns = document.querySelectorAll('.prev-step');
    const startStep = parseInt(enrollmentFlow?.dataset.currentStep || '1', 10);
